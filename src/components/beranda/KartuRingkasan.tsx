...
export function KartuRingkasan({ persen, dibuka, total, dimmed, tanya }: {
  persen: number; dibuka: number; total: number; dimmed: number; tanya: number;
}) {
  const angka = [
    { label: "Dibuka", nilai: <>{dibuka}<span className="text-sm text-white/60">/{total}</span></> },
  ...
  return (
    <article className="flex flex-col justify-between rounded-3xl bg-biru p-6 text-white sm:p-7">
      <div>
        <p className="mikro text-white/60">Ringkasan</p>
        <p className="mt-3 text-5xl font-semibold tracking-tight">{persen}%</p>
        <p className="mt-2 text-sm leading-relaxed text-white/80">{pesanProgres(persen)}</p>
      </div>
      <dl className="mt-8 grid grid-cols-3 gap-3 border-t border-white/15 pt-5">
        {angka.map((a) => (
          <div key={a.label}>
            <dt className="text-[11px] uppercase tracking-[0.08em] text-white/60">{a.label}</dt>
