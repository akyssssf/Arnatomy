...
export function PemilihOrgan({ organs, aktifId }: { organs: Organ[]; aktifId: number }) {
  return (
    <nav aria-label="Pilih organ" className="flex items-center gap-1 rounded-full bg-white p-1">
      {organs.map((o) => {
        const aktif = o.id_organ === aktifId;
        return (
          <Link key={o.id_organ} href={{ pathname: "/eksplorasi", query: { organ: o.id_organ } }}
            ...
            <Image src={o.gambar} alt="" width={28} height={28} className="h-7 w-7 rounded-full bg-white object-contain p-0.5" />
            {o.nama_organ}
          </Link>
        );
      })}
