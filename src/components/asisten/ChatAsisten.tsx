...
import { PertanyaanFormSchema, type BodyPart, type Organ } from "@/lib/schemas";
import { alert, bubble, input, kartu, tombol } from "@/lib/variants";
import { useUIStore } from "@/store/useUIStore";

const SARAN = ["Apa fungsi bagian ini?", "Di mana letaknya?", "Gangguan apa yang bisa terjadi?", "Apa bedanya dengan sisi jantung yang lain?"];
...
export function ChatAsisten({ organs, bagian, idBagianAwal }: {
  organs: Organ[]; bagian: BodyPart[]; idBagianAwal: number | null;
}) {
  ...
  const [idKonteks, setIdKonteks] = useState<number | null>(idBagianAwal);
  ...
  return (
    <>
      <div className={`${kartu({ padding: "md" })} mb-3`}>
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[12rem] flex-1">
            <label htmlFor="pilih-konteks" className="mikro mb-2 block">Konteks bagian tubuh</label>
            <select id="pilih-konteks" value={idKonteks ?? ""} onChange={(e) => setIdKonteks(e.target.value ? Number(e.target.value) : null)} className={input()}>
              <option value="">Tanpa konteks</option>
              {organs.map((o) => (
                <optgroup key={o.id_organ} label={o.nama_organ}>
                  {bagian.filter((b) => b.id_organ === o.id_organ).map((b) => (
                    <option key={b.id_bagian} value={b.id_bagian}>{b.nama_bagian_internal}</option>
                  ))}
                </optgroup>
              ))}
            </select>
          </div>
          <Link href={konteks ? { pathname: "/eksplorasi", query: { organ: konteks.id_organ } } : "/eksplorasi"}
            className={tombol({ variant: "sekunder", ukuran: "sm" })}>
            <Ikon nama="jantung" />Kembali ke model
          </Link>
        </div>
        <label className="mt-3 flex items-center gap-2 rounded-xl bg-abu px-3 py-2 text-xs text-neutral-500">
          <input type="checkbox" checked={simulasiGagal} onChange={(e) => setSimulasiGagal(e.target.checked)} className="h-4 w-4 accent-biru" />
      ...
      <div className={`${kartu({ padding: "md" })} mt-4 flex h-[26rem] flex-col sm:h-[30rem]`}>
        <ul ref={daftarRef} aria-live="polite" aria-atomic="false" aria-label="Riwayat percakapan dengan asisten AI"
          className="flex-1 space-y-2 overflow-y-auto pr-1">
          <Bubble peran="ai" isi={konteks
          ...
          {daftar.isPending && (
            <li className="flex items-end gap-2"><div className={bubble({ peran: "sistem" })}><Spinner kelas="h-3 w-3" /> Memuat riwayat percakapan</div></li>
          )}
          {daftar.isError && (
            <li className="flex items-end gap-2"><div className={bubble({ peran: "sistem" })}>Riwayat gagal dimuat: {daftar.error.message}</div></li>
          )}
          {daftar.data?.map((p) => (
          ...
          {kirim.isPending && kirim.variables && (
            <>
              ...
              <li className="flex items-end gap-2"><div className={bubble({ peran: "ai" })}><span className="text-xs text-neutral-400">Asisten sedang mengetik...</span></div></li>
            </>
          )}
        </ul>

        {galatForm && (
          <div role="alert" aria-live="assertive" className={`${alert({ tipe: pertanyaanGagal ? "error" : "info" })} mt-3 items-center justify-between`}>
            <span>{pertanyaanGagal ? `Gagal memuat jawaban: ${galatForm}` : galatForm}</span>
            {pertanyaanGagal && (
              <button type="button" onClick={() => kirimPertanyaan(pertanyaanGagal)} className={tombol({ variant: "sekunder", ukuran: "sm" })}>Coba lagi</button>
            )}
          </div>
        )}
        ...
        <ul className="mt-3 flex flex-wrap gap-2">
          {SARAN.map((s) => (
            <li key={s}>
              <button type="button" onClick={() => { setTeks(s); inputRef.current?.focus(); }}
                className="rounded-full bg-abu px-3 py-1.5 text-xs font-medium text-neutral-500 transition hover:bg-neutral-900 hover:text-white">{s}</button>
            </li>
          ))}
        </ul>
        ...
        <form onSubmit={saatSubmit} className="mt-3 flex items-end gap-2 border-t border-black/5 pt-3">
          <div className="flex-1">
            <label htmlFor="input-pertanyaan" className="sr-only">Tulis pertanyaan untuk asisten AI</label>
            <textarea id="input-pertanyaan" ref={inputRef} rows={2} value={teks} onChange={(e) => setTeks(e.target.value)}
              disabled={kirim.isPending} placeholder="Tulis pertanyaan, tekan Enter untuk mengirim"
