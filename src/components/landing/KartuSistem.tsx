...
export function KartuSistem({ sistem }: { sistem: SistemOrgan }) {
  ...
  return (
    <article className={`kartu-angkat group relative flex items-center gap-4 overflow-hidden rounded-2xl bg-white p-4 sm:p-5 ${tersedia ? "" : "opacity-80"}`}>
      <div className="grid h-24 w-24 shrink-0 place-items-center rounded-2xl bg-abu sm:h-28 sm:w-28">
        <Image src={sistem.gambar} alt="" width={96} height={96}
      ...
      <div className="min-w-0 flex-1">
        ...
        <Badge status={tersedia ? "tervalidasi" : "netral"} kelas="mt-3">{tersedia ? "Tersedia" : "Segera hadir"}</Badge>
      </div>
    </article>
  );
}
