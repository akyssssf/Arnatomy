...
export function HeroLogin({ organ, jumlahOrgan, jumlahBagian, jumlahLabel }: {
  organ: Organ; jumlahOrgan: number; jumlahBagian: number; jumlahLabel: number;
}) {
  return (
    <div className="mt-4 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
      <div>
        <h1 id="judul-login" className="titik-biru denyut text-[2.75rem] font-semibold leading-[0.92] sm:text-6xl lg:text-[5.25rem]">
          <JudulKata baris={[["Belajar", "anatomi"], ["lewat", "model", "3D"]]} />
        </h1>
        <Muncul jeda={350} kelas="mt-8 flex flex-wrap items-center gap-5">
          <a href="#form-login" className="inline-flex items-center gap-2 rounded-full bg-biru px-6 py-3 text-sm font-medium text-white transition hover:bg-biru-gelap">Mulai belajar</a>
